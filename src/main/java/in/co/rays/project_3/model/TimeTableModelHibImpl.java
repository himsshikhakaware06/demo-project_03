...
public class TimeTableModelHibImpl implements TimeTableModelInt{
	...
	public TimeTableDTO checkBySubjectName(long courseId, long subjectId, Date examDate)
		...
		try {
			...
			session = HibDataSource.getSession();
			Criteria criteria = session.createCriteria(TimeTableDTO.class).add(Restrictions.eq("courseId", courseId))
					.add(Restrictions.eq("subId", subjectId)).add(Restrictions.eq("examDate", date));
			List list = criteria.list();
			if (list.size() > 0) {
				dto = (TimeTableDTO) list.get(0);
			}
	...
	public TimeTableDTO checkBysemester(long courseId, long subjectId, String semester, Date examDate)
		...
		try {
			session = HibDataSource.getSession();
			Criteria criteria = session.createCriteria(TimeTableDTO.class).add(Restrictions.eq("courseId", courseId))
					.add(Restrictions.eq("subId", subjectId)).add(Restrictions.like("semester", semester))
			...
			/*
			  ...
			  dis.add(Restrictions.eq("examDate", date)); criteria.add(dis);
			*/
			List list = criteria.list();
			if (list.size() > 0) {
				dto = (TimeTableDTO) list.get(0);
